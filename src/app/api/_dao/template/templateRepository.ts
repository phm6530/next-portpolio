...
WITH RankedAnswers AS (
    SELECT 
        ...
        pr.gender,
        pr.age_group,
        ROW_NUMBER() OVER (PARTITION BY sq.id ORDER BY sa.id DESC) as rn -- 최신 답변이 상위로 정렬
    FROM 
        survey_question as sq
    LEFT JOIN
        survey_options as so
    ON
        sq.id = so.question_id AND sq.question_type_id != 'text'
    LEFT JOIN
        survey_answers as sa
    ON
        sq.id = sa.question_id AND (sa.answer_value = so.label OR sq.question_type_id = 'text')
    LEFT JOIN
...
SELECT 
    ...
    option_idx,
    option_pictrue,
    SUM(CASE WHEN gender = 'male' AND age_group = 10 THEN 1 ELSE 0 END) as male_10s,
    SUM(CASE WHEN gender = 'male' AND age_group = 20 THEN 1 ELSE 0 END) as male_20s,
    SUM(CASE WHEN gender = 'male' AND age_group = 30 THEN 1 ELSE 0 END) as male_30s,
    SUM(CASE WHEN gender = 'male' AND age_group = 40 THEN 1 ELSE 0 END) as male_40s,
    SUM(CASE WHEN gender = 'male' AND age_group = 50 THEN 1 ELSE 0 END) as male_50s,
    SUM(CASE WHEN gender = 'male' AND age_group = 60 THEN 1 ELSE 0 END) as male_60s,
    SUM(CASE WHEN gender = 'female' AND age_group = 10 THEN 1 ELSE 0 END) as female_10s,
    SUM(CASE WHEN gender = 'female' AND age_group = 20 THEN 1 ELSE 0 END) as female_20s,
    SUM(CASE WHEN gender = 'female' AND age_group = 30 THEN 1 ELSE 0 END) as female_30s,
    SUM(CASE WHEN gender = 'female' AND age_group = 40 THEN 1 ELSE 0 END) as female_40s,
    SUM(CASE WHEN gender = 'female' AND age_group = 50 THEN 1 ELSE 0 END) as female_50s,
    SUM(CASE WHEN gender = 'female' AND age_group = 60 THEN 1 ELSE 0 END) as female_60s
FROM 
    RankedAnswers
WHERE 
    rn <= 10 -- 각 질문별로 최신 10개의 답변만 선택
GROUP BY 
    question_id, type, question, text_answer, option_label, option_idx, option_pictrue
ORDER BY 
    question_id, rn;
  `;
